...
class Quiz : Thread {
    ...
    //runs the code
    override fun run() {
        //while loop that runs while the app is on
        while (true) {
            //reset checks if all 10 maps have been shown
            if (repeat == 10){
                repeat = 0
            }
            //populate list with indexes of flags
            if (flagsIndex.isEmpty()) {
                flagsIndex.addAll(countries.indices.shuffled().take(noFlags))
            }

            //use index to find name of flag and file name
            ...
            // sends flag name in lowercase and display "What Flag Is This?"
            ...
            // sends flag name in lowercase and display the country name
...
class QuizHandler: Runnable {
    ...
    override fun run() {
        ...
        // change image using fn and change text using text
